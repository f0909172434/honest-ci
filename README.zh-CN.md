...
发布状态：`v1.0.2`。1.x 公开接口保持加法兼容变更；可复现使用请固定 `v1.0.2`，moving `v1` 会跟随最新的兼容 1.x 版本。
...
```console
npm install --save-dev honest-ci@1.0.2
```
...
建议在 `baseline write` 后传入测试命令；只有命令成功且配置的报告确实更新时，HonestCI 才会写入基准。为兼容旧用法，命令仍可省略，但已有报告会带有 `HCI107_FRESHNESS_UNVERIFIED`。
